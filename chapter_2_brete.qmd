...
contributions in the signal [@Badgley et al.]  NIRv has been shown to have a
...
We used three deciduos broad leaf forest from the northern hemisphere with 
data processed by Ameriflux. For each site, three datasets were used with
GPP (GPP_DT_VUT_USTAR50) estimated by the ONEFlux processing at daily, weekly,
and monthly timeframes. Characteristics of these datasets are in 
@tbl-oneflux_datasets

The Bartlett experimental forest is located in New Hampshire, USA (44°06′N, 71°3′W). The site has a forest with a canopy mean height around 20-22m and a mean
annual temperature of 6C. Desiptes events such as a hurricane in 1938 and small
scale forest management, the forest mean stand age is around 120-125 years. [@ouimette_carbon_2018]
...
Southern Ontario which has been collecting EC data since 1996 [@rogers_response_2020].  This site has a forest cover of over 60% with a height that exceeds 2 m.
It’s a deciduous broadleaf natural re-growth forest dominated by woody 
vegetation. [@lee_long-term_1999].

The third site is the University of Michigan Biological Station which is
located in northern Michigan (45°350 N 84°430 W). The site have a forest with
different succesional stages and a mean height of 2m. Mean annual temperature
is around 5.5°C. [@gough_disturbanceaccelerated_2021]

### Satellite imagery

We obtained the data from the collections available in Google Earth Engine,
(See @tbl-satellite_images_datasets) for a defined square polygon with an
area of 3km around the EC tower for each site. For each polygon, the values of
all the bands for every pixel were used.

The MODIS Surface Reflectance product consist of 7 reflectance bands from
which three of them necessary to calculate the vegetation indices (See
@tbl- )

For the MODIS data (`MODIS/006/MOD09GA` and `MODIS/061/MOD09A1`) the
processing was split in three sections: filtering high quality pixels,
scaling band values and vegetation indices calculation. The filtering of the
highest quality pixels was done using the `state_1km` and `qc_500m` variables 
which are bit encoded. The variable `q_scan` was discarded given that provided
no useful information for the filtering process and `g_flags` contained the
same value across al the observations so it wasn't used.

To be able to use the bit encoded variables, 


 - modis surface reflectance have 7 bands (include bands ranges)
 - state_1km and QC_500m are bit encoded.
 -  
After MODIS data was downloaded, we processed both datasets (reflectance 500m
and 250m) first by doing a quality processing according to the categories
obtained through the bitwise convertion. After the filtering out of low quality
pixels, we scaled every band by a factor of `0.0001`. Any resulting value after
the scaling outside the range from `0` to `1` were discarded.

With the highest MODIS quality observations, we calculated the vegetation
indices NDVI, NIRv and EVI.

We filtered the pixels from Sentinel images according to the `msk_cldprb` and
`msk_snwprb` variables. After the filtering, we scaled all the observations by
a factor of `0.0001` and calculated the vegetation indices NDVI, NIRv, and EVI.
The bands used to calculate these indices are in @tbl-harmonized_s2_indices_bands
