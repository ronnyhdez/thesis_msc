...
We used three deciduos broad leaf forest from the northern hemisphere with data
processed by Ameriflux. For each site, three datasets were used with GPP
(`GPP_DT_VUT_USTAR50`) estimated by the ONEFlux processing at daily, weekly, and
monthly timeframes. Characteristics of these datasets are in
...
quality. Bit enconded variables were transformed to strings categories and just
...
provided no useful infomation about the quality and `g_flags` contained the same
...
and `EVI` (@eq-evi) were calculated. Given that in the MODIS product 
(`MODIS/061/MOD09A1`) which have a spatial resolution of 250m there is no blue
band, so `EVI` was not calculated for this dataset.
...
`msk_snwprb` variables. After the filtering, we scaled all the observations by a
factor of `0.0001` and calculated the vegetation indices NDVI, NIRv, and EVI.
