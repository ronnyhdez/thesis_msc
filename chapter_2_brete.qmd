...
calculation. The filtering of the highest quality pixels was done using the
`state_1km` and `qc_500m` variables which are bit encoded. The variable
`q_scan` was discarded given that provided no useful information for the
filtering process and `g_flags` contained the same value across all the 
observations so it wasn't used.
