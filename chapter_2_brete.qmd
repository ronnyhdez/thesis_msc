...
Two MODIS (Moderate Resolution Imaging Spectroradiometer) products were utilized:
`MODIS/006/MOD09GA` and `MODIS/061/MOD09A1`. The `MOD09GA` product has a spatial
resolution of 500m and consists of 7 reflectance bands, with 3 of them being 
used to calculate vegetation indices. The `MOD09A1` product has a spatial 
...
were filtered using four bit-encoded variables: `state_1km`, `qc_500m`, `q_scan`,
and `g_flags` along with each of the bands bits quality indicators. These 
variables provided information about the observation quality. The bit-encoded 
variables were transformed into categorical strings, and only the categories 
indicating the best quality were selected to filter the pixels. For filtering,
the `state_1km` and `qc_500m` variables were used, as `q_scan` was not 
informative and `g_flags` had the same value for all observations. The specific 
bit strings selected for `state_1km` are shown in @tbl-state_1km_bitstrings and
for `qc_500m` in @tbl-qc_scan_bit_strings.
...
In the study, three datasets were prepared for each site: a daily dataset, 
a weekly dataset, and a monthly dataset. These datasets were generated from the
...
representation of the variables, allowing for a detailed analysis of their 
daily fluctuations.
