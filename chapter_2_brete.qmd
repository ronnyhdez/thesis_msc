...
processing does the estimation of the CO~2~ fluxes into GPP and RECO from NEE 
through two methods known as daytime and nighttime. Here we selected the daytime
method (DT) which uses daytime and nighttime to parameterize a model with two
components: one based on light response curve and vapour pressure deficit and a
second one using a respiration-temperature relationship to estimate RECO which 
in turn is used to obtain the difference with NEE and provide GPP. [@pastorello2020fluxnet2015].
...
Gross Primary Production (GPP) estimation at each site, employing the vegetation
...
ind_bar <- bartlett_monthly_500 %>% 
  select(gpp_dt_vut_ref, evi_mean, ndvi_mean, nirv_mean) %>% 
...
#| fig-cap: "GPP observed and predicted values for all the sites. The dashed line represents a 1:1 relation."
#| echo: false
#| message: false
#| warning: false
...
## Collect predictions
plot_predictions(weekly_500_fit, metrics, 4, 4, 20, 21)
```
...
predictions_automl %>% 
  ggplot(aes(x = predict, y = gpp_dt_vut_ref)) +
  geom_text(aes(x = 3, y = 18,
                label = paste("rmse:", round(rmse, 3))),
            stat = "unique") +
  geom_text(aes(x = 3, y = 17,
                label = paste("rsq:", round(rsq, 3))),
            stat = "unique") +
  geom_abline(lty = 2) +
  geom_point(alpha = 0.5, size = 3, aes(shape = site)) +
  coord_fixed() +
