...
To build and evaluate the model, we performed a random data splitting procedure, dividing the data into training set with 70% of the observations and a test set
...
To understand which bands or VI's are the most important driving the predictions,
the global feature importance or variable importance was calculated (tidymodels). 
This process quantifies the influence of each feature on the model's predictive
performance, which helps to identify those critical variables for better 
predictions. To determine the scores, permutation technique was employed,
fitting the model with and quantifyng how the predictive performance of the
model deteriorates when a particular variables is permuted while keeping others
constant.
