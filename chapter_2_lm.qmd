...
compare the performance of the models we calculated the coefficient of 
determination (R2), the Root Mean Squared Error (RMSE), and the Mean Absolute 
Error (MAE)
...
Bayesian Information Criterion (BIC) are displayed in the @tbl-complete_lm_monthly_results
...
sites, respectively. In contrast, EVI exhibits less favourable 
predictive results for the Borden site and the aggregated sites category, although
in the specific context of the Borden site, its performance aligns closely with
NDVI and NIRv. Among the individual sites, Bartlett stands out with the most 
...
indices in Borden or the combined site is less than 50% in most of the cases
and the error scores being the highest. In the case of Michigan, the best 
performing individual index was EVI which outperformed the next best-performing 
individual index NIRv by 3%. Generally, the Bartlett and Michigan sites
consistently yielded the most accurate predictive models across various
configurations. Conversely, the Borden site consistently exhibited the poorest 
...
performance than those based on weekly or daily values as ilustrated in @fig-lm_metrics.
...
The @tbl-gam_model_results provides the results summary of the GAM models used 
for GPP estimation at each site, employing the vegetation indices as predictors.
To compare between the models, we include in the table relevant model summary 
...
Information Criterion (BIC) are dispalyed in the @tbl-complete_lm_monthly_results
...
the case of Michigan EVI and NIRv were the best individual indices with a 96%
of the variance in GPP, but EVI had slightly lower error metrics values in MAE 
...
also 4% but in this case the best individual performing VI was EVI. For Michigan,
the model using all VIs as covariates outperformed EVI by 5% in GPP prediction, 
and for Borden it was a 9% improvement when compared with NIRv.
...
most favourable results in terms of models explaining variance and yielding lower
residuals, followed by Michigan. In the case of Borden, when employing 
...
LM vs GAM
 ...
 - Monthly GAM Michigan had better results with EVI and NIRv than LM with all
 indices as covariates.
 - Weekly GAM models with all VIs as covariates got a better performance than
 the LM for all sites except for Borden that had the same R2 but nonetheless,
 lower error metrics.
 - This variance implied that a linear relationship might not be optimal, 
 potentially leading to substantial residuals. In contrast, the GAM model was
 found to adeptly accommodate such variability and provide a more comprehensive
 explanation for the relationship. This adaptability was especially significant
 when addressing GPP's daily, weekly, or monthly variations.
