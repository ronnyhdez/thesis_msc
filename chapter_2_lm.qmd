...
Our findings shows that when using all the indices as covariates within the linear model, the model's performance demonstrates better outcomes compared to using any single index 
alone across all scenarios. In the assessment of individual VI performance on a
monthly basis, CCI tend to perform better than EVI, NDVI, and NIRv. However it 
is important to note that for the Bartlett and Michigan sites, while CCI shows
favourable predictive accuracy, its advantage over the NIRv and EVI is slightly
better and mostly due to the MAE and RMSE results.
...
predictive results for the Borden site and the aggregated sites dataset, although
...
favourable predictive outcomes, while the aggregated sites dataset yield the 
...
performance than those based on weekly or daily values.
...
overfitting. Finally, it's noteworthy that NDVI displayed suboptimal performance
in two of the sites (Michigan and Bartlett) with a difference of 10% and 13%
with the best performing models respectively, while EVI exhibited less favorable
results in just the Borden site.
...
Daily:
 ...
 - Indices as covariates had a better performance than any other index used
 as a explanatory variable alone.
 - In the case of all sites, indices as covariates explained 4% more than CCI
 which was the index with better explanation power. In the case of Barlett that
 increase was of 4% compared with EVI. Michigan was 5% better at predicting GPP
 than EVI. In the case of Borden it was 9% compared with NIRv.
 - In the 4 cases, NDVI performed worst among all the individual indices.
 - As an individual index EVI performed better in just all the sites, and EVI
 in two cases (Bartlett and Michigan). NIRv was better in Borden as an individual
 index for Borden, nonetheles, the variance explained in this site was just of
 46%.
 - From the 3 individual sites Bartlett had the best results in terms of
 models explaingin variance and lower residuals, followed by Michigan. In the
 case of Borden, when using the individual indices, they could achieve a
 variance explanation less than 50%.
