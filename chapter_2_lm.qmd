...
individual indices such as NDVI and NIRv (R² = 1% variance explained and no more
...
superiority over the others. For the all sites category, CCI has an R² = 3%
...
with an R² = 96% of the variance in GPP, but EVI had slightly lower error metrics
values in MAE ($0.01 \, \mathrm{gC \, m^{-2} \, d^{-1}}$) and RMSE
($0.02 \, \mathrm{gC \, m^{-2} \, d^{-1}}$). Bartlett had a better model
performance when using CCI with an R² = 91% of variance explained with the lowest
...
indices as covariates yielded an R² = 7% increase in variance explanation
...
contrast to EVI, and an R² = 2% enhancement for Borden when compared with EVI.
...
reduction relative to EVI, and Borden exhibited an R² = 9% deficit when
contrasted with EVI.
...
the best performing individual VI CCI. In the case of Bartlett, the increase was
also R² = 4% but in this case, the best individual performing VI was EVI. For
Michigan, the model using all VIs as covariates outperformed EVI by R² = 5% in
GPP prediction, and for Borden, it was an R² = 9% improvement when compared with
NIRv.
