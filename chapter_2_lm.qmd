...
significant of the model fits. All models add p-values < 0.05 More metrics such as the p-value, adjusted
...
slightly better and mostly due to the MAE ($0.86 \, \mathrm{gC \, m^{-2} \, d^{-1}}$ 
for Bartlett and $1.02 \, \mathrm{gC \, m^{-2} \, d^{-1}}$ for Michigan) and 
RMSE  ($1.05 \, \mathrm{gC \, m^{-2} \, d^{-1}}$ for Bartlett and 
...
75%, MAE = $1.92 \, \mathrm{gC \, m^{-2} \, d^{-1}}$, 
RMSE = $2.54 \, \mathrm{gC \, m^{-2} \, d^{-1}}$) for the Borden site and the 
...
Bartlett has the most favourable predictive outcomes in terms of R²,
MAE, and RMSE for every individual VI model, while the aggregated sites category
yield the least favourable predictive results in the same scenarios.
...
as a single entity. However, for this case, EVI shows marginal differences with
respect to individual indices such as NDVI and NIRv (R² = 1% variance explained
and no more than $0.02 \, \mathrm{gC \, m^{-2} \, d^{-1}}$ in RMSE). The best
performant individual indices were CCI in terms of R², MAE, and RMSE for Borden
and the aggregated sites, NIRv for Michigan and EVI for Bartlett. Nonetheless,
those superior performances are subtle when compared with the other individual
indices. On a weekly basis, differences in variability explained between the
best and least performing models range from R² = 11% to R² = 20%. Bartlett and
...
for Bartlett by R² = 2% in variance explanation compared with EVI, for Borden by
R² = 1% compared with EVI, and for the combined sites dataset by R² = 10%.
Nonetheless is worth to mention that the variance explained in any of the models
by individual indices in Borden or the combined site is less than R² = 50% in
most of the cases and the error scores being the highest. In the case of
Michigan, the best performing individual index was EVI which outperformed the
next best-performing individual index NIRv by R² = 3%. Generally, the Bartlett
...
superiority over the others. For the all sites category, CCI have a R² = 3%
better variance explanation than NDVI, which is the second best model with a R²
= 75%. In the case of Michigan EVI and NIRv were the best individual indices
with a R² = 96% of the variance in GPP, but EVI had slightly lower error metrics
values in MAE ($0.01 \, \mathrm{gC \, m^{-2} \, d^{-1}}$) and 
RMSE ($0.02 \, \mathrm{gC \, m^{-2} \, d^{-1}}$). Bartlett had a better model 
performance when using CCI with a R² = 91% of variance explained with the lowest
...
= 10% and R² = 13% with the best performing models respectively, while EVI
exhibited less favourable results in just the Borden site with
MAE $1.92 \, \mathrm{gC \, m^{-2} \, d^{-1}}$) and 
RMSE $2.54 \, \mathrm{gC \, m^{-2} \, d^{-1}}$).
...
indices as covariates yielded a R² = 7% increase in variance explanation
compared with the best individual VI result CCI. Further, for Michigan, this
improvement amounted to R² = 2% compared to EVI, R² = 1% for Bartlett in
contrast to EVI, and a R² = 2% enhancement for Borden when compared with EVI.
...
yielded R² = 7% less variance explanation than CCI. Notably, for Michigan,
NDVI's performance lagged by R² = 17% compared to EVI, Bartlett an R² = 18%
reduction relative to EVI, and Borden exhibited a R² = 9% deficit when
contrasted with EVI.

On a daily basis, when considering the all sites category, the model with all
the VIs as covariates explained R² = 4% more variance in GPP when compared with
the best performing individual VI CCI. In the case of Bartlett, the increased
was also R² = 4% but in this case the best individual performing VI was EVI. For
Michigan, the model using all VIs as covariates outperformed EVI by R² = 5% in
GPP prediction, and for Borden it was a R² = 9% improvement when compared with
NIRv.
