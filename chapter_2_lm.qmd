...
@jung_towards_2009]. One widely used approach for approximating and quantifying
GPP involves employing data-driven models based on spectral vegetation indices
(VIs) [@ryu_what_2019] However, GPP estimation using spectral VIs remain with
high uncertainties, prompting the need for validation efforts are required to
identify approaches to improve accuracy. [@anav_spatiotemporal_2015;
@brown_fiducial_2021]
...
than \< 1km2 , which constrains the accuracy of estimating ecosystem carbon and
water fluxes at regional and global scales. [@badgley_canopy_2017].
...
type but also physiological conditions driven by water availability, temperature
stress, and vapour pressure deficit [@goetz_mapping_1999;
...
VI's have been used to provide inputs to @eq-gpp related to fAPAR
[@sellers_global_1994] or to directly estimate GPP such as normalized difference
vegetation index (NDVI), enhanced vegetation index (EVI), Near-Infrared
Reflectance Index (NIRv) and Chlorophyll/Carotenoid Index (CCI) among others.
...
photosynthetically active radiation (fAPAR) and LUE on
...
need for detailed knowledge about canopy and soil properties, relying instead on
measurements of reflected light with potential corrections for soil
...
aggregation of application. By isolating these factors, I aim to provide nuanced
understanding of the reliability and robustness of the VIs models in predicting
GPP for multiple growing seasons from the Bartlett Experimental Forest (USA),
the Borden Forest Research Station flux-site (Canada), and the University of
...
MOD09GA Version 6.1 product (MODIS/Terra Surface Reflectance Daily L2G Global 1
km and 500 m SIN Grid). A square polygon with an area of 3km surrounding the EC
tower was defined for each site, and the complete data pixel values within this
polygon was extracted for analysis.
...
variables were used along with each of the bands bits quality indicators, as
...
@tbl-MODIS_500_indices_bands). The resulting values used for the analysis are
shown in @fig-quality_pixels
...
significant of the model fits. More metrics such as the p-value, adjusted
r-squared, the Akaike Information Criterion (AIC), and Bayesian Information
Criterion (BIC) are displayed in the @tbl-complete_lm_monthly_results for
monthly results, @tbl-complete_lm_weekly_results for weekly results, and the
...
In summary, our analysis demonstrates that the incorporation of all VIs as
covariates in the model yields enhanced predictive accuracy for GPP. This
observation suggests that the synergistic utilization of all indices
collectively provides a better capacity to elucidate and capture the nuanced
variations in GPP throughout the entire study duration when contrasted with the
predictive power of any individual VI.
...
employing distinct models for each site. This diminished performance may be
attributed to the introduction of variability inherent in the Borden site data,
characterized by a substantial range in GPP values that proves challenging to
encapsulate accurately. Further examination of GPP trends across Michigan and
Bartlett, juxtaposed with those of Borden, reveals notably wider data ranges,
particularly during periods of higher productivity @fig-gpp_trends.
...
more robust and accurate global GPP estimations. For example, the study by [@lin_evaluating_2019] showed that Chlorophyll Index Red (CRI) had a better correlation with
GPP, or CCI for evergreen conifers [@gamon_remotely_2016].

The application of linear models, incorporating all Vegetation Indices (VIs) as
covariates on a monthly basis, produced favorable outcomes in both model fit and
residual metrics. This observation holds significance as, despite VIs serving as
indicators of vegetation capacity rather than vegetation physiology, historical
data records are predominantly derived from sensors equipped with bands for
generating these VIs. Conversely, newer sensors capable of capturing additional
bands to derive indices such as Solar-Induced Fluorescence (SIF) lack
comprehensive historical records, hindering the feasibility of long-term studies
that extend back to years preceding the 2000s.

While NDVI is one of the most used VIs in EO [@pabon-moreno_potential_2022],
its performance, as measured by the RMSE, was comparatively less favorable on
...
pertinent question arises concerning the significance of the time range in GPP
estimation. Our results indicate that models based on monthly data demonstrated
better model fit for each model and smaller residuals metrics. This superiority
may be attributed to reduced data variation, as daily values are aggregated into
monthly summaries, leading to values more centralized around a mean. The
performance of model fits and residuals metrics declined when transitioning from
monthly to weekly and, subsequently, to daily values. Although the decrease in
model fit was not substantial, the increase in residuals metrics suggests a
potential for larger errors in predictions.

When examining the data trends for GPP at each site and its relationship with
Vegetation Indices (VIs), it becomes evident that at higher GPP values, the
dispersion of the data is more pronounced. This observed pattern may be ascribed
to the inherent limitation of these indices, which predominantly track the
presence of green leaves. It is important to note, however, that the mere
presence of green leaves does not consistently signify active photosynthesis.
This discrepancy can arise for two reasons. Firstly, the photosynthetic process
may undergo temporary suspension without any manifest changes in chlorophyll
content or leaf abscission  [@camps-valls_unified_2021-2] Secondly, VIs relying
on the NIR band face challenges in detecting photosynthesis in instances when a
higher amount of healthy plant biomass is present. The increased biomass leads
to greater scattering and reflection of NIR radiation, resulting in a saturation
effect [@camps-valls_unified_2021-2].
