...
When using all VIs as covariates, we obtained better results than any other 
model with an individual VI, for every site category and timeframe evaluated.
In the assessment of individual VI performance on a monthly basis, CCI tend to 
perform better than EVI, NDVI, and NIRv. However it is important to note that 
for the Bartlett and Michigan sites, while CCI shows favourable predictive
accuracy, its advantage over the NIRv and EVI is slightly better and mostly due
to the MAE and RMSE results.
...
sites, respectively. On the other hand, the EVI exhibits less favourable 
...
In the case of the weekly models, NDVI yielded the least favourable results when
...
NIRv for  Michigan and EVI for Bartlett. Nonetheless, those best performances 
are subtle when compared with the other individual indices. At a weekly basis, 
differences between the best model and the less performant model can go from 11%
up to 20% in the variability explanation.



Intriguingly, in the case of the Bartlett site, EVI exhibited superior GPP prediction, with minimal variance from the Chlorophyll/Carotenoid Index (CCI) except for a slight discrepancy in the Root Mean Square Error (RMSE). Conversely, the Borden site exhibited the poorest individual model performance, and when the data from all sites were combined, the models fared even less favorably. In contrast, both the Bartlett and Michigan sites independently yielded the most accurate predictive models.
...
In our analysis, the Chlorophyll/Carotenoid Index (CCI) emerged as the top-performing index in three instances, specifically for the Bartlett, Borden, and the combined dataset of all sites, with Borden slightly outperforming the others by a margin of 1%. As an individual index, the Enhanced Vegetation Index (EVI) excelled, particularly in the case of the Michigan site, where it outperformed the next best-performing individual index by a notable 3%. Across all scenarios, the incorporation of all vegetation indices as covariates consistently contributed to enhanced model performance. Generally, the Bartlett and Michigan sites consistently yielded the most accurate predictive models across various configurations. Conversely, the Borden site consistently exhibited the poorest model performance across all scenarios.
