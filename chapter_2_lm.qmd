...
techniques [@ryu_what_2019; @wang_diagnosing_2011]. Satellite data-driven
models can be classified into process-based models, Light Use Efficiency 
models (LUE), and Vegetation Index models [@xie_assessments_2020].
...
As such, the main objective of this M.Sc. thesis is the analysis of uncertainty
associated with the VI/LUE models to estimate GPP. These models eliminate the 
need for detailed knowledge about canopy and soil properties, relying instead
on measurements of reflected light with potential corrections for soil 
contribution. The focal point of my research is to quantify the uncertainty
inherent in these models, particularly when the variation in environmental
condictions (E) is controlled. This control is achieved by selecting sites that 
share the same land cover, climate, and biome characteristics. However, 
considering the reviewed studies, we hypothesize that the uncertainty of this
approach will depend on the nature of the VI and the spatial and temporal
aggregation of application. By isolating these factors, I aim to provide
nuanced understanding of the reliability and robustness of the VIs models in 
predicting GPP for multiple growing seasons from the Bartlett Experimental
Forest (USA), the Borden Forest Research Station flux-site (Canada), and the
University of Michigan Biological Station (USA).
...
validation of cloud-free pixels as well. Bands used to derived the vegetation
...
@tbl-MODIS_500_indices_bands). The resulting values used for the analysis are shown in
@fig-quality_pixels
...
daily basis for the time period of available GPP at each site @tbl-oneflux_datasets. 
This dataset provided a high-resolution representation of the
...
statistics, such as  R2, MAE, and RMSE as indicators of the statistical 
significant of the model fits. More metrics such as the p-value,
...
counterparts, as illustrated in @fig-gam_metrics and @fig-lm_metrics. This can
be an effect of reducing variance when summarizing values as it is shown in the
daily @fig-gpp_vi_relation_daily, weekly @fig-gpp_vi_relation_weekly,
and monthly @fig-gpp_vi_relation_monthly 
